...
def register(request):
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            send_email.send()
         #   messages.success(request,'Thank you for registering for us. We have sent you a verification email to your email addess.please verify it.')
...
def my_orders(request):
    orders = Order.objects.filter(user=request.user, is_ordered=True).order_by('-created_at') # Giving result in decending order -created_at
    context = {
        'orders':orders,
    }
...
def change_password(request):
    if request.method=='POST':
        ...
        if new_password==confirm_password:
            success=user.check_password(current_password) #check django
            if success:
                user.set_password(new_password)  #set django
                user.save()
                #auth.logout(request)  # after password change user is logged out
